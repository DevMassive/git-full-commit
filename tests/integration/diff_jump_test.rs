...
fn test_diff_jump_from_stat_summary() {
    ...
    let (stat_line_index, patch_start_index) = {
        ...
        let stat_line_index = first_file.lines.iter().position(|l| l.contains('|')).expect("Should find a stat line");
        let patch_start_index = first_file.lines.iter().position(|l| l.starts_with("diff --git")).expect("Should find patch start");
...
fn test_diff_jump_multiple_files() {
    ...
    let (second_stat_index, expected_offset) = {
        ...
        let stat_indices: Vec<usize> = first_file.lines.iter().enumerate()
